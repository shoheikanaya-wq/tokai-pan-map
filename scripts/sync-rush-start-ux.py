...
#settingsPanel>strong{display:block;font-size:15px;margin-bottom:3px}
@media(max-width:560px){.easy-guide{font-size:9px;padding:8px}}
</style>'''
...
# Make the custom setup read like a simple sequence.
...
# Recommended start should skip the confusing setup screen and go directly to candidate loading.
...
# After candidates load, take the user to the next step automatically.
...
# Sync Service Worker cache because Pan Rush behavior/UX changed.
sw = Path('sw.js')
...
# Keep home annotation and cache-buster aligned with the simpler flow.
idx = Path('index.html')
idx_text = idx.read_text(encoding='utf-8')
idx_text = re.sub(r'panrush\.html\?v=[^"\']+', 'panrush.html?v=20260908-easy', idx_text, count=1)
idx_text = idx_text.replace('⚡ パンラッシュは「おまかせで始める」または条件指定から直感的に開始できます。', '⚡ パンラッシュは「すぐ始める」ならおすすめ設定で候補店まで自動で進みます。自分で設定する場合も4項目だけです。')
